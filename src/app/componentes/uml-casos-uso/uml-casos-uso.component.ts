...
export class UmlCasosUsoComponent implements OnInit {
  diagram!: go.Diagram;
  ...
  initDiagram() {
    ...
    this.diagram.linkTemplateMap.add('extend',
      $(go.Link, { routing: go.Link.AvoidsNodes, adjusting: go.Link.Stretch,corner: 5, relinkableFrom: true, relinkableTo: true, reshapable: true},
        ...
        $(go.TextBlock, '<<extend>>', {segmentFraction: 0.5, segmentOffset: new go.Point(-20, -10), font: "bold 20px sans-serif", stroke: "blue", editable: true})
      )
    );

    // Adicion de la relacion include
    ...
    console.log("Diagrama de casos de uso inicializado");
  }
  ...
  initPalette() {
    ...
    console.log("Paleta de casos de uso inicializada");
  }
  ...
  asociacion() {
    ...
    this.diagram.toolManager.linkingTool.archetypeLinkData = { category: 'normal' };
    console.log("Modo de Asociación Normal")
  }
  ...
  guardarComoImagen(diagram: go.Diagram) {
    ...
    pdf.save("diagrama-casos-uso.pdf");
  }
  ...
  cargadoConExito() {
    this.toastr.success('Diagrama Cargado con Éxito', 'Nice');
  }
