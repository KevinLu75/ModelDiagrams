...
export class UmlSecuenciasComponent implements AfterViewInit, OnInit {
  ...
  initDiagram() {
    ...
    // Listeners para asignar puertos dinámicamente
    ...
    this.loadModel();
    console.log("Diagrama de secuencias inicializado");
  }
  ...
  initPalette() {
    ...
    console.log("Paleta de secuencias inicializada");
  }
  ...
  cargarDiagrama() {
    ...
    if (jsonData) {
      ...
      this.toastr.error('No hay diagrama guardado', 'Error');
    }
  }

  eliminarElemento() {
    const selectedParts = this.myDiagram.selection;
    this.myDiagram.startTransaction('delete');
    selectedParts.each((part) => {
      this.myDiagram.remove(part);
    });
  ...
  guardarComoImagen(diagram: go.Diagram) {
    ...
    pdf.save("diagrama-secuencias.pdf");
  }
